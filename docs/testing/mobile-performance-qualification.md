...
fixtures still carry an explicit terminal outcome. The scorer requires exact
fixture coverage and unique trial/artifact/timestamp identities. It also rejects
duplicate-equivalent measurement payloads, so relabelling one scene's telemetry
299 times cannot satisfy coverage.

Record first-install transfer, cached storage, camera-preview baseline memory,
Then conduct three non-overlapping runs of at least ten minutes. Each has a
...
overlapping run spans, reused artifact hashes, and duplicate-equivalent telemetry
even when caller-supplied run IDs differ. Every raw sample is expressed in the
unit named by the evidence schema; memory and transfer values are MiB.
...
Empty trial measurements, incomplete start/run/checkpoint coverage, duplicate or
copied evidence, wrong fixture coverage, mismatched profile/device/browser
metadata, or an absent performance block make the profile ineligible. Malformed
numeric evidence is rejected rather than silently scored.
